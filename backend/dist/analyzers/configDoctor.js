...
exports.analyzePortsAndDocker = analyzePortsAndDocker;
exports.analyzeCi = analyzeCi;
exports.analyzeDocumentation = analyzeDocumentation;
exports.analyzeRuntime = analyzeRuntime;
...
// ─── Helpers ──────────────────────────────────────────────────────────────────
function issue(severity, confidence, category, title, description, recommendedFix, extras = {}) {
    return {
        id: (0, uuid_1.v4)(),
        severity,
        confidence,
...
async function analyzeEnvVars(repoPath, sourceFiles) {
    ...
    const envFiles = await (0, fast_glob_1.default)(['.env.example', '.env.sample', '.env.template', '.env.defaults'], { cwd: repoPath, absolute: false, dot: true });
    for (const relPath of envFiles) {
        ...
        for (const name of parseEnvNames(content)) {
            ...
            if (line && line.split('=').slice(1).join('=').trim())
                defaults.add(name);
        }
    }
...
async function analyzePortsAndDocker(repoPath, profile) {
    ...
    for (const df of dockerfiles) {
        const content = (0, security_1.readFileSafe)(path_1.default.join(repoPath, df));
        if (!content)
            continue;
        ...
        if (/dockerfile$/i.test(df)) {
            if (!nonCommentContent.includes('npm ci') && /npm\s+install/.test(nonCommentContent)) {
                issues.push(issue('low', 0.7, 'docker', `Dockerfile uses \`npm install\` instead of \`npm ci\``, `In \`${df}\`, using \`npm install\` in production Docker images can produce non-deterministic builds. Prefer \`npm ci\`.`, `Replace \`npm install\` with \`npm ci\` in your Dockerfile.`, { filePath: df, evidence: ['RUN npm install found in Dockerfile'] }));
            }
            ...
            if (!hasNonRootUser) {
                issues.push(issue('medium', 0.8, 'docker', `Docker container may run as root`, `No non-root USER directive found in \`${df}\`. Running containers as root is a security risk.`, `Add \`USER node\` (or another non-root user) before CMD/ENTRYPOINT in your Dockerfile.`, { filePath: df, evidence: ['No USER directive found in Dockerfile'] }));
            }
        }
        // Check: docker-compose missing required env vars from .env.example
        if (/docker-compose\.(yml|yaml)$/i.test(df)) {
            const envExampleContent = (0, security_1.readFileSafe)(path_1.default.join(repoPath, '.env.example'))
                ?? (0, security_1.readFileSafe)(path_1.default.join(repoPath, '.env.sample'))
                ?? '';
            ...
            if (requiredVars.length > 0) {
                ...
                const missingInCompose = requiredVars.filter((v) => !composeVars.has(v) && (0, security_1.isSecretName)(v));
                if (missingInCompose.length > 0) {
                    issues.push(issue('high', 0.85, 'docker', `docker-compose is missing required secret environment variable(s)`, `\`${df}\` does not define: ${missingInCompose.join(', ')}. These are listed in \`.env.example\` as required secrets and the application will fail to start without them.`, `Add the missing variable(s) to the \`environment:\` block in \`${df}\`. Use Docker secrets or a secrets manager — never commit real values.`, { filePath: df, evidence: missingInCompose.map((v) => `${v} missing from docker-compose environment`) }));
                }
            }
        }
    }
...
// ─── CI analysis ──────────────────────────────────────────────────────────────
async function analyzeCi(repoPath, profile) {
    const issues = [];
    const ciFiles = profile.configFiles.filter((f) => f.includes('.github/workflows') || f.endsWith('.gitlab-ci.yml') || f.includes('.circleci/'));
    let anyWorkflowRunsTests = false;
    const testScriptNames = Object.keys(profile.scripts).filter((k) => k === 'test' || k.startsWith('test'));
    for (const ciFile of ciFiles) {
        const content = (0, security_1.readFileSafe)(path_1.default.join(repoPath, ciFile));
        if (!content)
            continue;
        ...
        if (hasTestInCi) {
            ...
            if (!hasInstall) {
                issues.push(issue('high', 0.9, 'ci', `CI workflow may not install dependencies`, `\`${ciFile}\` runs tests but does not appear to run a dependency install command first.`, `Add \`run: npm ci\` (or equivalent) to your CI workflow before the test step.`, { filePath: ciFile, evidence: ['No npm/yarn/pnpm install found in workflow running tests'] }));
            }
            const nodeVersionMatch = content.match(/node-version[:\s]+['"]?(\d+)/i);
            if (!nodeVersionMatch) {
                if (profile.nodeVersion) {
                    issues.push(issue('low', 0.7, 'ci', `CI workflow does not pin a Node.js version`, `\`${ciFile}\` runs tests but does not specify a Node.js version. Without pinning, CI may behave differently from local development.`, `Add \`node-version: '${profile.nodeVersion}'\` to your workflow's setup-node step.`, { filePath: ciFile }));
                }
            }
                ...
                if (requiredMatch) {
                    ...
                    if (ciNode < requiredNode) {
                        issues.push(issue('high', 0.9, 'ci', `CI uses Node.js ${ciNode} but project requires Node.js ${requiredNode}+`, `\`${ciFile}\` uses \`node-version: '${ciNode}'\` but \`package.json\` engines field requires Node.js ${profile.nodeVersion}.`, `Update the CI workflow to use \`node-version: '${requiredNode}'\` (or higher).`, { filePath: ciFile, evidence: [`CI node-version: ${ciNode}, engines: ${profile.nodeVersion}`] }));
                    }
                }
            }
        }
    ...
    if (!anyWorkflowRunsTests && testScriptNames.length > 0 && ciFiles.length > 0) {
        issues.push(issue('high', 0.85, 'ci', `Tests are not run in CI`, `The project has a test script defined, but none of the CI workflows run it.`, `Add a step that runs \`npm test\` (or equivalent) in your main CI workflow.`, { evidence: [`Test script exists: ${testScriptNames.join(', ')}`] }));
    }
    return { issues, ciFiles };
}
// ─── Documentation analysis ───────────────────────────────────────────────────
function analyzeDocumentation(repoPath, profile) {
    const issues = [];
    const readmeFile = profile.configFiles.find((f) => /readme\.md$/i.test(f));
    if (!readmeFile) {
        issues.push(issue('low', 1.0, 'documentation', 'No README.md found', 'The repository does not have a README.md file. This makes it difficult for new contributors to understand the project.', 'Create a README.md with setup instructions, run commands, and test commands.'));
        return issues;
    }
    const content = (0, security_1.readFileSafe)(path_1.default.join(repoPath, readmeFile)) ?? '';
    ...
    if (!lower.includes('npm install') && !lower.includes('yarn install') && !lower.includes('pnpm install')) {
        issues.push(issue('info', 0.8, 'documentation', 'README does not document how to install dependencies', `\`${readmeFile}\` does not include dependency installation instructions.`, 'Add installation instructions (e.g., `npm install`) to your README.', { filePath: readmeFile }));
    }
    ...
    if (hasTestScript && !lower.includes('npm test') && !lower.includes('npm run test') && !lower.includes('yarn test')) {
        issues.push(issue('info', 0.75, 'documentation', 'README does not document how to run tests', `\`${readmeFile}\` does not mention the test command despite a test script being defined.`, 'Add test instructions (e.g., `npm test`) to your README.', { filePath: readmeFile }));
    }
    ...
    if (readmePorts.size > 0) {
        ...
        for (const relPath of profile.sourceFiles) {
            if (!/\.(ts|js)$/.test(relPath))
                continue;
            const src = (0, security_1.readFileSafe)(path_1.default.join(repoPath, relPath));
            if (!src)
                continue;
            let am;
        ...
        if (appPorts.size > 0) {
            ...
            if (mismatchedReadmePorts.length > 0) {
                issues.push(issue('medium', 0.75, 'documentation', `README documents port(s) not used by the application`, `\`${readmeFile}\` mentions port(s) ${mismatchedReadmePorts.join(', ')} but application code uses port(s) ${[...appPorts].join(', ')}. This will mislead developers trying to connect to the service.`, `Update \`${readmeFile}\` to reference the correct port(s): ${[...appPorts].join(', ')}.`, { filePath: readmeFile, evidence: mismatchedReadmePorts.map((p) => `Port ${p} in README not found in source code`) }));
            }
        }
    }
    return issues;
...
function analyzeRuntime(repoPath, profile) {
    const issues = [];
    // Missing test script
    if (!profile.scripts['test']) {
        issues.push(issue('medium', 0.95, 'scripts', 'No "test" script defined in package.json', 'The project does not have a `test` script in package.json. Running `npm test` will fail.', 'Add a `"test"` script to package.json (e.g., `"test": "jest"` or `"test": "vitest run"`).', { filePath: 'package.json' }));
    }
    // Has test framework in devDeps but no test script
    const hasTestFrameworkDep = ['jest', 'vitest', 'mocha'].some((fw) => profile.devDependencies[fw]);
    if (hasTestFrameworkDep && !profile.scripts['test']) {
        issues.push(issue('medium', 0.9, 'scripts', 'Test framework installed but no test script configured', 'A test framework is listed in devDependencies but no `test` script is defined.', 'Configure the test script in package.json to use the installed framework.', { filePath: 'package.json', evidence: ['Test framework in devDependencies but no test script'] }));
    }
    // .env.example exists check
    const hasEnvExample = (0, security_1.fileExists)(path_1.default.join(repoPath, '.env.example')) ||
        (0, security_1.fileExists)(path_1.default.join(repoPath, '.env.sample'));
    const usesEnv = profile.sourceFiles.length > 0; // simplified check — full check is done in env analysis
    if (!hasEnvExample && usesEnv) {
        issues.push(issue('low', 0.7, 'environment', 'No .env.example file found', 'The project does not have a .env.example file. New contributors will not know which environment variables are required.', 'Create a .env.example file documenting all required environment variables (with placeholder values, not real secrets).'));
    }
    return issues;
}
// ─── Full ConfigDoctor ────────────────────────────────────────────────────────
async function runConfigDoctor(repoPath, profile) {
    ...
    return {
        ...
        hasEnvExample: (0, security_1.fileExists)(path_1.default.join(repoPath, '.env.example')) || (0, security_1.fileExists)(path_1.default.join(repoPath, '.env.sample')),
        hasDotenv: profile.devDependencies['dotenv'] !== undefined || profile.dependencies['dotenv'] !== undefined,
        summary,
    };
}
//# sourceMappingURL=configDoctor.js.map