    ...
    for (const ciFile of ciFiles) {
        ...
        if (!nodeVersionMatch) {
            if (profile.nodeVersion) {
                issues.push(issue('low', 0.7, 'ci', `CI workflow does not pin a Node.js version`, `\`${ciFile}\` does not specify a Node.js version. Without pinning, CI may behave differently from local development.`, `Add \`node-version: '${profile.nodeVersion}'\` to your workflow's setup-node step.`, { filePath: ciFile }));
            }
        }
        ...
        if (!/npm (ci|install)|yarn install|pnpm install/.test(content)) {
            issues.push(issue('high', 0.9, 'ci', `CI workflow may not install dependencies`, `\`${ciFile}\` does not appear to run a dependency install command.`, `Add \`run: npm ci\` (or equivalent) to your CI workflow.`, { filePath: ciFile, evidence: ['No npm/yarn/pnpm install found in workflow'] }));
        }
        ...
        if (!hasTestInCi && testScriptNames.length > 0) {
            issues.push(issue('high', 0.85, 'ci', `CI workflow does not run tests`, `\`${ciFile}\` does not appear to run the test suite, but the project has a test script defined.`, `Add a step that runs \`npm test\` (or equivalent) in your CI workflow.`, { filePath: ciFile, evidence: [`Test script exists: ${testScriptNames.join(', ')}`] }));
        }
    }
