...
exports.detectCoverage = detectCoverage;
exports.analyzeTests = analyzeTests;
exports.categorizeEnvVar = categorizeEnvVar;
...
function countTestsInText(text, filePath) {
...
function detectCoverage(repoPath) {
    ...
    const summaryJsonPath = path_1.default.join(repoPath, 'coverage', 'coverage-summary.json');
    if (fs_1.default.existsSync(summaryJsonPath)) {
        try {
            const content = fs_1.default.readFileSync(summaryJsonPath, 'utf8');
    ...
    const lcovPaths = [
        path_1.default.join(repoPath, 'coverage', 'lcov.info'),
        path_1.default.join(repoPath, 'lcov.info'),
    ];
            ...
            try {
                ...
                if (lfMatches && lhMatches) {
                    for (const m of lfMatches)
                        linesFound += parseInt(m.slice(3), 10);
                    for (const m of lhMatches)
                        linesHit += parseInt(m.slice(3), 10);
                    if (linesFound > 0) {
                        const pct = Math.round((linesHit / linesFound) * 100);
                        return {
                            status: 'ACTUAL_COVERAGE',
                            percentage: pct,
                            reason: `Parsed actual coverage from LCOV report at ${(0, security_1.relativePath)(repoPath, lp)} (${pct}% lines covered)`,
                        };
                    }
                }
            }
...
function analyzeTests(repoPath, testFiles) {
    ...
    for (const rel of filesToScan) {
        ...
        const content = (0, security_1.readFileSafe)(abs);
        if (!content)
            continue;
...
function categorizeEnvVar(name) {
    ...
    if (OS_SHELL_SET.has(upper) || upper.length <= 2)
        return 'OS_SHELL';
    // 3. Node Runtime
